...
/**
 * \brief Loads CTPPSDAQMapping and CTPPSAnalysisMask from two XML files.
 **/
...
public:
  ...
  edm::ESProducts< boost::shared_ptr<CTPPSDAQMapping>, boost::shared_ptr<CTPPSAnalysisMask> > produce( const CTPPSDAQMappingRcd & );
...
private:
  ...
  /// parses XML file
  void ParseXML(ParseType, const string &file, const boost::shared_ptr<CTPPSDAQMapping>&, const boost::shared_ptr<CTPPSAnalysisMask>&);
  ...
  void ParseTreeRP(ParseType, xercesc::DOMNode *, NodeType, unsigned int parentID,
    const boost::shared_ptr<CTPPSDAQMapping>&, const boost::shared_ptr<CTPPSAnalysisMask>&);
...
private:
  ...
  CTPPSFramePosition ChipFramePosition(xercesc::DOMNode *chipnode);
  ...
  currentBlockValid(false)
{
  ...
  findingRecord<CTPPSDAQMappingRcd>();
}
...
edm::ESProducts< boost::shared_ptr<CTPPSDAQMapping>, boost::shared_ptr<CTPPSAnalysisMask> >
  TotemDAQMappingESSourceXML::produce( const CTPPSDAQMappingRcd & )
{
  assert(currentBlockValid);

  boost::shared_ptr<CTPPSDAQMapping> mapping(new CTPPSDAQMapping());
  boost::shared_ptr<CTPPSAnalysisMask> mask(new CTPPSAnalysisMask());
...
void TotemDAQMappingESSourceXML::ParseXML(ParseType pType, const string &file,
  const boost::shared_ptr<CTPPSDAQMapping> &mapping, const boost::shared_ptr<CTPPSAnalysisMask> &mask)
{
...
void TotemDAQMappingESSourceXML::ParseTreeRP(ParseType pType, xercesc::DOMNode * parent, NodeType parentType,
  unsigned int parentID, const boost::shared_ptr<CTPPSDAQMapping>& mapping,
  const boost::shared_ptr<CTPPSAnalysisMask>& mask)
{
#ifdef DEBUG
  ...
  for (unsigned int i = 0; i < children->getLength(); i++)
  {
    ...
    {
      const CTPPSFramePosition &framepos = ChipFramePosition(n);
      CTPPSVFATInfo vfatInfo;
      vfatInfo.hwID = hw_id;
...
CTPPSFramePosition TotemDAQMappingESSourceXML::ChipFramePosition(xercesc::DOMNode *chipnode)
{
  CTPPSFramePosition fp;
